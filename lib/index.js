/**
 * dsh-fail-logger v0.4.4 — all-mode tool failure recorder (DeepSeek Harness).
 *
...
export function apply(ctx, config = {}) {
  ...
  const ensureSkillFile = () => {
    ...
    try {
      ...
      writeAtomic(SKILL_FILE, ['---', 'name: ' + nameOk, 'description: 工具失败自动实录——记录近期工具调用失败的原因、次数与规避建议；当工具调用失败、报错、重试受阻时加载本技能查看历史错因，避免重复犯错', '---', ''].join('\n') + '\n');
    } catch {}
  };
