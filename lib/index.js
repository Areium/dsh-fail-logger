/**
 * dsh-fail-logger v0.4.0 — all-mode tool failure recorder (DeepSeek Harness).
 *
...
export function apply(ctx, config = {}) {
  ...
  const todayKey = () => fmtLocal(new Date().toISOString()).slice(0, 10);

  const redact = (s) => { for (const [re, rep] of REDACT) s = s.replace(re, rep); return s; };
  // Markdown 消毒：剥控制字符、转义表格竖线与行首 #
  const mdSafe = (s) => redact(String(s).replace(/[\x00-\x1f\x7f]/g, ' ')).replace(/\|/g, '\\|').replace(/^#/gm, '\\#');

  const normMessage = (msg) => {
    const s = toText(msg).split(/\r?\n/)[0].trim().slice(0, MAX_MSG);
  ...
  const loadState = () => {
    try {
      const s = sanitize(JSON.parse(readFileSync(STATE_FILE, 'utf8')));
      if (s) return s;
  ...
  const loadStateTolerant = () => {
    try { return sanitize(JSON.parse(readFileSync(STATE_FILE, 'utf8'))); } catch { return null; }
  };
  ...
  const renderSection = (state) => {
    ...
    for (const cat of order) {
      const rows = (groups[cat] || []).sort(rankCompare).slice(0, MAX_ENTRIES);
  ...
  const flush = () => {
    ...
    try {
      const merged = mergeStates(loadStateTolerant(), memState);
      memState = merged;
      dirty = false;
      writeAtomic(STATE_FILE, JSON.stringify(merged, null, 2));
  ...
  const record = (kind, message, meta = {}) => {
    try {
      ...
      memState.days[todayKey()] = (memState.days[todayKey()] || 0) + 1;
      // TTL：长期无新发生的条目归档删除
      if (TTL_DAYS > 0) {
        const cutoff = Date.now() - TTL_DAYS * 86400e3;
        for (const [k, v] of Object.entries(memState.entries)) {
          if (new Date(v.last).getTime() < cutoff) delete memState.entries[k];
        }
      }
      // 状态有界：超出上限裁剪低频旧条目
      const all = Object.values(memState.entries);
      if (all.length > MAX_ENTRIES * 5) {
        const keep = new Set(all.sort(rankCompare).slice(0, MAX_ENTRIES * 3).map((e) => hashOf(e.kind, e.message)));
  ...
  const handle = (_session, event) => {
    try {
      ...
      if (event.type === 'tool/result') {
        ...
        recordToolFailure(info?.name ?? 'unknown', blk ? blk.content : m.content, { code: event.data?.error?.code, args: info?.args });
        return;
      }
      if (event.type === 'tool/code-dispatch') {
        const d = event.data;
        if (!d || d.isError !== true) return;
        recordToolFailure(d.name ?? 'unknown', d.content, { args: d.arguments });
      }
    } catch {}
  };
