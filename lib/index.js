 ...
 * ~/.dsh/skills/fail-log-guide/SKILL.md) — deduplicated by message, counted,
 ...
 * Env override for testing: FAIL_LOG_DIR (PTC_FAIL_LOG_DIR kept for compatibility).
 *
...
export function apply(ctx, config = {}) {
  ...
  // tool/result 的 content 可能是结构化对象：取 .message 字段或序列化，绝不产出 "[object Object]"
  const normMessage = (msg) => {
    let s;
    if (typeof msg === 'string') s = msg;
    else if (msg && typeof msg === 'object') {
      if (typeof msg.message === 'string') s = msg.message;
      else { try { s = JSON.stringify(msg); } catch { s = String(msg); } }
    } else s = String(msg ?? '');
