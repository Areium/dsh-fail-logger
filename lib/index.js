/**
 * dsh-fail-logger v0.5.5 — all-mode tool failure recorder (DeepSeek Harness).
 *
 ...
 *   ttlDays    - archive entries with no new occurrence for N days (default 30, 0 = keep).
 *                Archived entries stay in the ledger (counts preserved), are hidden from
 *                the rendered section, and revive automatically on recurrence.
 *   topErrorsRefresh - 'session' (default): snapshot the top-errors block at startup only; it refreshes again on the next session/restart (cache-friendly). 'flush': refresh after every flush (legacy behavior).
...
// 注意：edit 三连（file changed / old_string not found / requires reading）刻意不在此列——
// 实测其占近期失败约 70% 且静态规则未能压住，必须保留在 top-errors 动态段中提供复发可见性。
const STATIC_PREVENTION_RE = [
  /unknown tool .*only `run_code` is callable/i,
  // 仅匹配「路径不存在」类（消息形如 `cannot read "X": not found`），
  // 不能写成裸 /not found/——会误伤 `old_string was not found` 把它错误地过滤掉
...
export function apply(ctx, config = {}) {
  ...
  // 二级(top-errors)刷新模式:'session'=启动快照后会话内冻结(默认,避免 system prompt 中段变化击穿 LLM 前缀缓存),新会话/重启时自动读盘更新;'flush'=每次落盘后刷新(旧行为)
  const TOP_ERRORS_REFRESH = config.topErrorsRefresh === 'flush' ? 'flush' : 'session';
  ...
  const migrateState = (state) => {
    ...
    // v2 → v3：合并「DSH 引入 error.code 前/后」的同 message 双键条目。
    // 键从 hashOf(kind|msg) 漂移到 hashOf(kind:code|msg) 后，同一错误会裂成两条、
    // 复发计数被摊薄；这里把无 code 的旧键并入有 code 的新键。
    for (const [k, v] of Object.entries(out.entries)) {
      if (!v || typeof v.message !== 'string' || typeof v.code !== 'string' || !v.code) continue;
      const legacyKey = hashOf(v.kind, normKey(v.message));
      const prev = out.entries[legacyKey];
      if (prev && legacyKey !== k && prev.code === undefined) {
        v.count += prev.count;
        v.first = v.first < prev.first ? v.first : prev.first;
        v.last = v.last > prev.last ? v.last : prev.last;
        if (v.args === undefined && prev.args !== undefined) v.args = prev.args;
        delete out.entries[legacyKey];
      }
    }
    return out;
  };
  ...
  const mergeStates = (disk, mem) => {
    ...
    for (const [k, v] of Object.entries(mem.entries)) {
      if (out.entries[k]) {
        ...
        // message 与 args 同源成对更新：仅在新 occurrence 带来 args 时一起替换，
        // 避免「message 停留在首次、args 却是最新」的张冠李戴（渲染出的命令与错误信息错位）
        if (v.args !== undefined) { out.entries[k].args = v.args; out.entries[k].message = v.message; }
        if (v.code !== undefined) out.entries[k].code = v.code;
        delete out.entries[k].archived;   // TTL 归档条目一旦复发即复活（保留历史累计 count）
      } else out.entries[k] = { ...v };
    }
    for (const [d, c] of Object.entries(mem.days || {})) out.days[d] = (out.days[d] || 0) + c;
    out.totalEvents = (typeof out.totalEvents === 'number' ? out.totalEvents : 0) + (mem.totalEvents || 0);
    return out;
  };
  ...
  const renderSection = (state) => {
    ...
    for (const e of Object.values(state.entries)) {
      if (e.archived) continue;   // TTL 归档条目隐藏不渲染（数据仍保留在 .failures.json）
    ...
    try {
      ...
      try {
        if (Date.now() - statSync(LOCK_FILE).mtimeMs > LOCK_STALE_MS) {
          // 回收陈旧锁前验证持锁进程存活（锁内容为 pid）：进程仍活跃则说明它只是慢，
          // 直接 unlink 会造成两个进程同时自持锁、后写者基于旧读回写（条目丢失）
          let alive = false;
          try {
            const pid = parseInt(readFileSync(LOCK_FILE, 'utf8'), 10);
            if (Number.isFinite(pid)) {
              try { process.kill(pid, 0); alive = true; }
              catch (e) { alive = e?.code === 'EPERM'; }   // EPERM = 进程存在但无权发信号
            }
          } catch { alive = false; }   // 锁文件已消失/不可读 → 可安全回收
          if (!alive) { try { unlinkSync(LOCK_FILE); } catch {} }
        }
      } catch {}
      return false;
    }
  ...
  let timer = null;
  let dirty = false;
  let lastWrittenTotal = null;   // 本进程最近一次成功落盘的 totalEvents，用于检测外部重置/丢失
  const flush = () => {
    ...
    try {
      ...
      // 防丢对账（轻量）：disk 上的累计事件数低于本进程上次落盘值 → 状态被外部重置/截断，告警而非静默
      if (disk && typeof disk.totalEvents === 'number' && lastWrittenTotal !== null && disk.totalEvents < lastWrittenTotal) {
        console.warn('[dsh-fail-logger] WARNING: ledger regression detected — disk totalEvents=' + disk.totalEvents + ' < last written ' + lastWrittenTotal + ' (state was reset/truncated outside this process; check .failures.json backups)');
      }
      // 旧账本没有 totalEvents 字段：以 days 合计为基线收养（只作一致性校验基准，不回填历史条目）
      if (disk && typeof disk.totalEvents !== 'number') {
        disk.totalEvents = Object.values(disk.days || {}).reduce((a, b) => a + (typeof b === 'number' ? b : 0), 0);
      }
      const merged = mergeStates(disk, memState);
      // TTL：长期无新发生的条目归档（保留累计计数、渲染隐藏、复发即复活），不再物理删除——
      // 修复「趋势线有数、实录无线」与复发计数从 1 重来的割裂
      if (TTL_DAYS > 0) {
        const cutoff = Date.now() - TTL_DAYS * 86400e3;
        for (const [k, v] of Object.entries(merged.entries)) {
          if (new Date(v.last).getTime() < cutoff) v.archived = true;
        }
      }
      // 状态有界：超出上限裁剪低频旧条目（fallback 键必须复刻 record 的真实键：含 code、过 normKey，否则旧条目会被误删）
      const all = Object.values(merged.entries);
      if (all.length > MAX_ENTRIES * 5) {
        const keep = new Set(all.sort(rankCompare).slice(0, MAX_ENTRIES * 3).map((e) => e.key ?? hashOf(e.code ? e.kind + ':' + e.code : e.kind, normKey(e.message))));
      ...
      // 一致性校验：days 合计不应超过累计事件数（超过说明 days 在、entries/totalEvents 丢了）
      const daySum = Object.values(merged.days).reduce((a, b) => a + b, 0);
      if (typeof merged.totalEvents === 'number' && daySum > merged.totalEvents) {
        console.warn('[dsh-fail-logger] WARNING: days total (' + daySum + ') exceeds totalEvents (' + merged.totalEvents + ') — ledger diverged (possible entry loss); consider restoring .failures.json from backup');
      }
      // 落盘前剥掉内部 key 字段（保持状态文件干净）
      const out = { schemaVersion: STATE_SCHEMA_VERSION, pluginVersion: VERSION, updatedAt: new Date().toISOString(), totalEvents: merged.totalEvents, entries: {}, days: merged.days };
      ...
      lastWrittenTotal = typeof out.totalEvents === 'number' ? out.totalEvents : lastWrittenTotal;
      writeSection(renderSection(merged));
      // 会话内冻结:二级(top-errors)只取启动快照(loadDiskState),会话中不再重渲染——新会话启动时自动读盘更新
      if (TOP_ERRORS_MAX > 0 && TOP_ERRORS_REFRESH === 'flush') topSummary = renderTopErrors(merged);
  ...
  const record = (kind, message, meta = {}) => {
    ...
    try {
      ...
      // message 跟随最新 occurrence（与下方的 args 同源），避免跨次错位的「张冠李戴」
      const entry = prev
        ? { ...prev, count: prev.count + 1, last: now, message: msg }
      ...
      memState.totalEvents = (memState.totalEvents || 0) + 1;
