/**
 * dsh-fail-logger v0.5.0 — all-mode tool failure recorder (DeepSeek Harness).
 *
...
const PROMPT_TEXT = "【写操作铁律 · dsh-fail-logger】每次写操作前自查三点：1) 覆盖已有文件先 read（否则报 without reading it first）；2) 模板字符串不嵌 Python/Shell 代码——脚本用 write 落盘再执行；3) 模块/测试引用用 new URL('./x', import.meta.url)（+ fileURLToPath()），不硬编码路径。遇到工具失败/报错/重试受阻，用 skill 工具加载 fail-log-guide 查看历史错因，避免重复犯错。";
