 ...
 * is backed up before reset, entries are pruned to stay bounded, the runtime is
 * wrapped at most once, and record/write errors never break a code run.
 */
...
const WRAPPED = Symbol.for('ptc-fail-logger.wrapped');
...
export function apply(ctx, config = {}) {
  ...
  if (!runtime || typeof runtime.run !== 'function' || runtime[WRAPPED]) return;
  ...
  const ensureSkillFile = () => {
    ...
    try {
      ...
      writeAtomic(SKILL_FILE, ['---', 'name: ' + nameOk, 'description: 自动记录的 PTC 失败实录（由 ptc-fail-logger 维护）', '---', ''].join('\n') + '\n');
    } catch {}
  };
  ...
  const flush = () => {
    ...
    try {
      ...
      console.error('[ptc-fail-logger] flush failed:', e?.message ?? e);
    }
  };
  const schedule = () => {
    dirty = true;
  ...
  const record = (kind, message) => {
    try {
      ...
      console.error('[ptc-fail-logger] record failed:', e?.message ?? e, e?.stack?.split('\n')[1]);
    }
  };
